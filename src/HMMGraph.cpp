...
void HMMGraph::buildGraph(){
    ...
    contigLength = (ind_prec)length(contig)+1;
    numberOfStates = GRAPH_SIZE(contigLength, params.maxInsertion);
    seqGraph = new SeqNode[numberOfStates];

    contigChar = new char[contigLength];
    contigChar[0] = '\0';
    //@IMPORTANT: contigLength should be length(contig). Check ranges here in general.
    for(ind_prec curChar = 1; curChar <= contigLength; ++curChar)
        contigChar[curChar] = contig[curChar-1];
