 ...
 */
template <typename T>
inline void findMaxValues(const T* values, bool* selectedIndices, const ind_prec startValues,
                          const ind_prec endValues, const size_t maxValuesSize){
    ...
    for(ind_prec curState = startValues; curState < endValues; ++curState){
...
inline void chunkReads(const seqan::Dna5String& readSeq, ind_prec pos,
                       const seqan::String<seqan::CigarElement<> >& cigar, cnt_prec chunkSize, cnt_prec maxGraphSize,
    ...
    for(ind_prec i = 0; i < length(cpyCigar); ++i){
        ...
        if(type == 'S') seqan::erase(readStr, curIndex, curIndex+cpyCigar[i].count);
            ...
            if(length(curChunkedStr) == chunkSize && curEnd-curStart <= maxGraphSize){
    ...
    //@IMPORTANT: changed for GPU performance rasons. It was > 0
    if(length(curChunkedStr) == chunkSize && curEnd-curStart <= maxGraphSize){
        reads.push_back(Read(curChunkedStr, curStart, curEnd));
    }
}
...
* If you are going to change the transition structure, change it from there. These are imaginary edges in the graph.
*/
inline void insertNewBackwardTransitions(std::vector<TransitionInfoNode>* transitionSet, 
                                         const SeqNode& node, const cnt_prec numberOfDeletions, 
                                         const cnt_prec maxInsertion){

    if(node.getCharIndex() == 0) return;

    if(node.isMatchState()){
        for(ind_prec offset = 1; offset <= (ind_prec)numberOfDeletions+1 && 
            offset <= node.getCharIndex(); ++offset){
            
            transitionSet->push_back(TransitionInfoNode(node.getIndex(), 
                                     MATCH_DOFFSET(node.getCharIndex(), offset, maxInsertion)));
                                     //deletion
            
            for(cnt_prec curInsertion = 1; curInsertion <= maxInsertion; ++curInsertion){
                transitionSet->push_back(TransitionInfoNode(node.getIndex(), 
                                         INSERTION_DOFFSET(node.getCharIndex(), offset, 
...
inline bool fillBuffer(seqan::BamFileIn& alignmentFileIn, const seqan::FaiIndex& readsIndex,
                       ...
                       cnt_prec mapQ, ind_prec size, cnt_prec chunkSize, cnt_prec maxGraphSize){
        ...
        if(!hasFlagUnmapped(record) && !hasFlagLast(record) && record.mapQ >= mapQ){
            ...
            if(getIdByName(qId, readsIndex, record.qName)){
                ...
                if(length(alignedString) > 0 && record.beginPos >= 0){
                    chunkReads(alignedString, record.beginPos+1, record.cigar, chunkSize, maxGraphSize, reads);
                }
            }
        }
